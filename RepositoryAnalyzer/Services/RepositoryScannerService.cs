...
namespace RepositoryAnalyzer.Services;
...
public class RepositoryScannerService
{
    ...
    private async Task BuildDependencyGraphAsync(Scan scan, string repositoryPath)
    {
        _logger.LogInformation("Building dependency graph...");

        // Build solution-to-project dependencies
        await BuildSolutionProjectDependenciesAsync(scan, repositoryPath);

        // Build project-to-project dependencies
        await BuildProjectProjectDependenciesAsync(scan);

        // Build assembly-to-assembly dependencies
        await BuildAssemblyDependenciesAsync(scan);

        _logger.LogInformation("Dependency graph completed");
    }

    private async Task BuildSolutionProjectDependenciesAsync(Scan scan, string repositoryPath)
    {
        var solutions = await _context.Solutions.Where(s => s.ScanId == scan.Id).ToListAsync();
        var projects = await _context.Projects.Where(p => p.ScanId == scan.Id).ToListAsync();
        
        var projectByPath = projects.ToDictionary(p => p.FilePath, StringComparer.OrdinalIgnoreCase);

        foreach (var solution in solutions)
        {
            try
            {
                var content = await File.ReadAllTextAsync(solution.FilePath);
            ...
            {
                _logger.LogError(ex, "Error building dependencies for solution: {FilePath}", solution.FilePath);
            }
        }
    ...
    private async Task BuildProjectProjectDependenciesAsync(Scan scan)
    {
        var projects = await _context.Projects.Where(p => p.ScanId == scan.Id).ToListAsync();
        var projectByPath = projects.ToDictionary(p => p.FilePath, StringComparer.OrdinalIgnoreCase);
        ...
        foreach (var project in projects)
        {
            try
            {
                ...
                foreach (var reference in projectReferences)
                {
                    ...
                    var absolutePath = Path.GetFullPath(Path.Combine(projectDir!, includePath));
            ...
            {
                _logger.LogError(ex, "Error building dependencies for project: {FilePath}", project.FilePath);
            }
        }
    ...
    private async Task BuildAssemblyDependenciesAsync(Scan scan)
    {
        ...
        foreach (var assembly in assemblies)
        {
            try
            {
                if (!projectByPath.TryGetValue(assembly.ProjectFilePath, out var sourceProject))
                    continue;

                var content = await File.ReadAllTextAsync(sourceProject.FilePath);
                var projectDir = Path.GetDirectoryName(sourceProject.FilePath);
                ...
                foreach (var reference in projectReferences)
                {
                    ...
                    if (assemblyByProjectPath.TryGetValue(absolutePath, out var targetAssembly))
                    {
                        ...
                        _context.AssemblyDependencies.Add(dependency);
                    }
                }
            }
            catch (Exception ex)
            {
                _logger.LogError(ex, "Error building assembly dependencies for: {FilePath}", assembly.ProjectFilePath);
            }
        }
